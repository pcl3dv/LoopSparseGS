	...
	const bool debug)
{
  ...
  torch::Tensor out_weights = torch::full({1, H, W}, 0, int_opts);
  torch::Tensor max_weights = torch::full({1, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_weights.contiguous().data<int>(),
		max_weights.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, out_weights, max_weights, radii, geomBuffer, binningBuffer, imgBuffer);
}
